...
prompt.start();
prompt.get(['amount', 'tokenM','path','pathT'], function (err, result) {
    ...
    path = tokenFirstPath;//Array.from(tokenFirstPath.split(','));
    console.log('  CurrencyToken1: ' , path);
    let tokenSecondPath = result.pathT;
    pathTwo = tokenSecondPath;//Array.from(tokenSecondPath.split(","));
    console.log("CurrencyToken2:-",pathTwo)
    tokenM = result.tokenM;//Array.from(tokenSecondPath.split(","));
    console.log("Token:-",tokenM)
    init(amount,tokenM,path,pathTwo);
  });
...
const init = async(amountIn,tokenM,path,pathT) => {
