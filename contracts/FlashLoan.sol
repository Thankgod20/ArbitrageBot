...
contract BakeryFlashswap is IUniswapV2Callee {
    ...
    function initFlashloan(address _tokenBorrow, uint amount,address[] calldata path1,address[] calldata path2,uint tokenToBorrow) external{
        ...
        bytes memory data = abi.encode(_tokenBorrow,amount,path1,path2);
    ...
    function uniswapV2Call(address _sender,uint _amount0,uint _amount1, bytes calldata _data) external override {
        ...
        (address _tokenBorrow, uint amount,address[] memory path1,address[] memory path2) = abi.decode(_data,(address, uint,address[],address[]));
        //
